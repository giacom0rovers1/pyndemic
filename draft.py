...
er.degseq = sorted([d for n, d in er.degree()], reverse=True)  # degree sequence
plt.hist(er.degseq, density=True)


ba = nx.barabasi_albert_graph(N, k, seed)
ba.degseq = sorted([d for n, d in ba.degree()], reverse=True)  # degree sequence
...
ws.degseq = sorted([d for n, d in ws.degree()], reverse=True)  # degree sequence
...
plt.hist(ws.degseq, align = "left", bins = x_int, color = "lightsteelblue", label = "Degree (k)", density = True)
