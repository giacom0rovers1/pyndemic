\begin{table}[h]
\centering
\caption{Network properties: $\langle k \rangle$, $\langle C \rangle$ and $\langle l \rangle$ are the average node connectivity degree, the average clustering coefficient and the average shortest path length respectively. $K_0$ and the critical value are presented in pairs: the number estimated from eigenvalues $\Lambda_M$ on the left, the approximation for uncorrelated networks on the right.}  %  [$eigenv./uncorr.$]
\label{tab:networks}
