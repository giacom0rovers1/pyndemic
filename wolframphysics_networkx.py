...
run = 0
nl = 0
while nl < 100:  # keeps an eye on memory and visual repr.
    # while run < 100:
    run += 1
    print('Run =', run)
    ...
    for i, j in edges:
        ...
        for ii, k in edges:
            ...
            if (i == ii and j != k):
                ...
                new_edges.append((i, nl))
                new_edges.append((j, nl))
                new_edges.append((k, nl))
                nl += 1
                # break
                # continue
    print('')
...
# colors=range(20)
# cmap=plt.cm.Blues
# nx.draw(S, pos, node_color='#A0CBE2', edge_color=colors, width=4,
#         edge_cmap=cmap, with_labels=False, vmin=vmin, vmax=vmax)
